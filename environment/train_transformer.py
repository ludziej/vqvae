...
def get_model(ckpt_dir, restore_ckpt, **params):
    ...
    print(f"Restoring prior from {last_path}" if last_path else f"Starting training from scratch")
...
def train(hparams):
    ...
    generic_train(prior, hparams, train_dataloader, test_dataloader, "prior")
