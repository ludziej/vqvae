...
def create_vqvae(sample_length, l_mu, from_last_checkpot, ckpt_dir, restore_ckpt, **params):
    all_params = {"input_shape": (sample_length, 1), "mu": l_mu, **params}
    last_path = get_last_path(ckpt_dir, restore_ckpt) if from_last_checkpot else None
...
def calc_metaparams(dataset, forward_params, duration):
...
def get_model(sample_len, data_depth, sr, train_path, forward_params, band_est_dur, with_train_data=False, **params):
    ...
    print(train_data)
    calc_metaparams(train_data, forward_params, band_est_dur)
...
def train(hparams):
    ...
    generic_train(model, hparams, train_dataloader, test_dataloader, "prior")