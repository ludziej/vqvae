...
find -L "$1" -type f \( -iname "*.ape" -o -iname "*.wav"  -o -iname "*.mp3" -o -iname "*flac" \) | cat -n | while read n f; do mkdir -p "`dirname "$2/$f"`"  ;  ! test -f "$2"/"$f".wav && sleep 1.2 ; ffmpeg  -n -i "$f" -ar 44100 "$2"/"$f".wav </dev/null & done
