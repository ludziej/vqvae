...
python3 run_training.py --gpus=[0] --config=config_big --model=compressor --train_path=resources/fma_wav --compressor=big_vae \
	...
	--compressor.log_interval=500 --track_grad_norm=2 --compressor.ckpt_freq=500 --compressor.logger_type="neptune" \
	--compressor.batch_size=6 --compressor.num_workers=4 --compressor.sample_len=262144 --compressor.use_audiofile=0 --accelerator=ddp \
       	--compressor.lr=0.001 --compressor.lr_decay=5000 --compressor.lr_gamma=0.7 --compressor.emb_width=8 \
