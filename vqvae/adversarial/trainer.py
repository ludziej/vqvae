...
from vqvae.adversarial.model import JoinedDiscriminator, MelDiscriminator, WavDiscriminator
...
def get_discriminator(with_discriminator, type, **params):
    ...
    if type == "joined":
        ...
        return MelDiscriminator(**params)
    elif type == "wav":
...
class AdversarialTrainer(nn.Module):
    ...
    def forward(self, x_in, gen_out, optimize_generator=False):
        ...
        loss = sum([x[0] for x in calced_stats])
        metrics = ChainMap(*[self.discriminator_metrics(optimize_generator, *stats) for stats in calced_stats])
        return loss, metrics
    ...
    def discriminator_metrics(self, optimize_generator, loss, acc, bs, cls, y_true, loss_ew, disc_name="discriminator"):
        prefix = ("generator" if optimize_generator else disc_name) + "_"
        metrics = {prefix + "loss/total": loss}
        ...
        for level, lvl_loss in enumerate(loss_ew):
            metrics[f"{prefix}loss/lvl_{level}"] = torch.mean(lvl_loss) if optimize_generator else \
                (torch.mean(lvl_loss) + torch.mean(loss_real))/2

        if not optimize_generator:  # log per level acc stats
            metrics[f"{prefix}acc/balanced"] = acc
            error = (y_true == cls).reshape(self.levels + 1, bs).float()
            real_acc = torch.mean(error[-1])  # accuracy on real input
            metrics[f"{prefix}acc/on_real"] = real_acc
            for i in range(self.levels):
                fake_acc = torch.mean(error[i])  # accuracy on fake input on given level
                metrics[f"{prefix}acc/lvl_{i + 1}"] = (real_acc + fake_acc) / 2  # average with real to balance
                metrics[f"{prefix}acc/on_fake_lvl_{i + 1}"] = fake_acc
        return metrics
