...
class LevelGenerator(LightningModule):
    def __init__(self, vqvae: VQVAE, level: int, log_sample_size: int, context_on_level: int,
                 ...
                 pos_init_scale: int, bins_init_scale: float, separete_up_level_emb: bool,
                 conds_kwargs: dict):
        super().__init__()
        self.n_ctx = n_ctx
        self.level = level
        ...
        self.separete_up_level_emb = separete_up_level_emb
        self.transformer = PerformerLM(causal=True, num_tokens=self.bins, dim=dim, depth=depth, heads=heads,
                                       max_seq_len=n_ctx)
        ...
        nn.init.normal_(self.x_emb.weight, std=0.02 * self.bins_init_scale)
        if self.separete_up_level_emb:
            self.up_lvl_emb = nn.Embedding(self.bins, self.dim)
            nn.init.normal_(self.up_lvl_emb.weight, std=0.02 * self.bins_init_scale)
        else:
            self.up_lvl_emb = self.x_emb
    ...
    def forward(self, sound: torch.Tensor, context=None, time=None) -> torch.Tensor:
        ...
        embedding = self.get_conditioned_input(tokens, up_tokens, context, time)
    ...
    def get_lvl_conditioning(self, up_level, bs, length):
        up_level_emb = self.up_lvl_emb(up_level)
        contextual = self.conditioner(x, )
        raise NotImplementedError("currently no lvl conditioning")
    ...
    def get_conditioned_input(self, tokens, up_tokens=None, context=None, time=None):
        embeddings = self.x_emb(tokens)
        conditioning = self.get_all_conditionings(tokens.shape[0], tokens.shape[1], context=context,
                                                   up_tokens=up_tokens, time=time)
    ...
    def join_conditioning(self, x, lvl_cond=None, context_cond=None, pos_cond=None, time_cond=None,
        ...
        is_moving_window = [False, True, True, True]
        conds = [(c, m) for c, m in zip(conds, is_moving_window) if c is not None]
        conds = [c if token_interv is None else c[token_interv[0]: token_interv[1]] if moving else c[:size]
    ...
    def autoregressive_generate_prior(self, start_tokens, seq_len, conditioning=None, **sampling_kwargs):
        ...
        for i in range(t, seq_len):
            ...
            logits = self.transformer(x_emb_cond)[:, -1, :]
